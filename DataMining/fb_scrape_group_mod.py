...
# I would like to datamine from the UT Electrical & Computer Engineering group
# Its group ID is 155621761221818 (determined from something)

socket.setdefaulttimeout(30)
id_file = 'input.csv'  # change to your input filename
clientid = '1717436308485253'  # replace with actual client id
# replace with actual client secret
clientsecret = 'eb5e20cbaa15066019cf7917c5e9bf58'
outfile = 'fb_page_posts.csv'  # change the output filename if you like

# I made an access token that can actually see the group.
# The app's access token actually can't, since it doesn't have access to
# UT Facebook groups.
access_token = 'CAAYZAZC80XdIUBAEXOfZAkVx98XAZAfEPqe2TT2haUVIZC2aekDNPuqgDkuoMIAThWT0NZCd4' \
    'wUOSbAZArZBAqR2tOkV5qXQbiTZAQcTP2UfZAsI7l' \
    'w8ECZAit6qEEHYLlX4VCaEFmY2VOqTq9D28vqOT16PppkcMH' \
    'YZB7aPREG2q79AozubKNygbbgFHT3WUV1hwpvmX0XQqD61au0ffe' \
    'Mc3W4r6j9oie7iMsAZD'
    ...
    if type(data) is str:
        ...
        with open(data, 'r', encoding=enc, errors='replace') as f:
            reader = csv.reader((line.replace('\0', '')
                                 for line in f))  # remove NULL bytes
            for row in reader:
                if row != []:
                    csv_data.append(row)
        return csv_data
...
# this assumes a list of lists wherein the second-level list items contain
# no commas
def save_csv(filename, data, quotes_flag='', file_mode='w', enc='utf-8'):
...
def make_csv_chunk(fb_json_group, scrape_mode, thread_starter='', msg=''):
    csv_chunk = []
    if scrape_mode == 'feed':
        for line in fb_json_group['data']:
...
fb_urlobj = urllib.request.urlopen('https://graph.facebook.com/oauth/access_token=' +
                                   access_token + '&client_secret=' + clientsecret + '&client_id=' + clientid)
...
for x, fid in enumerate(fb_ids):
    if '_' in fid:
        ...
        msg_url = 'https://graph.facebook.com/v2.4/' + fid + '?' + fb_token
    ...
    data_url = 'https://graph.facebook.com/v2.4/' + \
        fid + '/' + scrape_mode + '?limit=1000&' + fb_token
    next_item = url_retry(data_url)
    csv_data = csv_data + \
        make_csv_chunk(next_item, scrape_mode, msg_user, msg_content)
    n = 0
    ...
    while 'paging' in next_item and 'next' in next_item['paging']:
        ...
        csv_data = csv_data + \
            make_csv_chunk(next_item, scrape_mode, msg_user, msg_content)
        try:
            print(n, next_item['data'][len(
                next_item['data']) - 1]['created_time'], time.time() - time1, 'seconds elapsed')
        except IndexError:
            break
        n += 1
        time.sleep(1)
